...
impl Task {
    pub async fn load(fullpath: PathBuf) -> Result<Task, NodeError> {
        ...
        let package_xml = match tokio::fs::read(&package_xml).await {
            ...
            Err(e) => return Err(NodeError::create_fs_error(&package_xml, e)),
        };
            ...
            match current_path {
                Some(current_path) => {
                    if (&current_path).join("package.xml").exists() {
                        ...
                        let errmap = NodeError::create_fs_errmap(&current_path);
    ...
    pub async fn list() -> NodeResult<Vec<Task>> {
        let tasks = tokio::task::spawn_blocking(Self::find_all_paths).await??;
