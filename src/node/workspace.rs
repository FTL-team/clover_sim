...
use super::{
    cmd::execute_cmd,
    err::{InvalidWorkspaceKind, NodeError, NodeResult},
};
...
impl Workspace {
        ...
        match FileLock::lock(path) {
            ...
            Err(err) => {
                return Err(match err.kind() {
                    ...
                    _ => NodeError::create_fs_error(&path.join("_.lock"), err),
                })
            }
        }
    }

    async fn load_workspace_meta(
        path: &PathBuf,
        name: &str,
    ) -> NodeResult<WorkspaceSerialized> {
    ...
    pub async fn list() -> NodeResult<Vec<String>> {
        tokio::task::spawn_blocking(|| {
            ...
            let err_mapper = NodeError::create_fs_errmap(&ws_path);
    ...
    pub async fn export(&self) -> NodeResult<PathBuf> {
        let target_path = self.path.join(EXPORT_NAME);

        if let Err(e) = tokio::fs::File::create(&target_path).await {
            match e.kind() {
                std::io::ErrorKind::AlreadyExists => todo!(),
                _ => return Err(NodeError::create_fs_error(&target_path, e)),
            }
        }
        ...
        cmd.arg("-zcf").arg(EXPORT_NAME).arg(".");
    ...
    pub async fn import_workspace(from: PathBuf) -> NodeResult<String> {
        let target_extract =
            PathBuf::from(WORKSPACES_DIR).join(format!("__import_{:04x}", rand::random::<u16>()));

        tokio::fs::create_dir(&target_extract)
            .await
            .map_err(NodeError::create_fs_errmap(&target_extract))?;

        let mut cmd = tokio::process::Command::new("tar");
        cmd.arg("-xf").arg(from).arg("-C").arg(&target_extract);
        cmd.stdin(Stdio::null());
        ...
        tokio::fs::rename(target_extract, &final_path)
            .await
            .map_err(NodeError::create_fs_errmap(&final_path))?;
