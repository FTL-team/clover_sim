...
use crate::{clean::Cleaner, cmd::simple_cmd, errhelper::IoNodeError, workroot::WorkEntry};
...
impl Network {
    ...
    async fn write_hosts(&self) -> NodeResult<()> {
        ...
        tokio::fs::write(self.hosts_entry.path(), host_s.as_bytes())
            .await
            .map_err(IoNodeError::create_fs_errmap(self.hosts_entry.path()))?;
        Ok(())
    }
    ...
    pub async fn new(
        ...
        tokio::fs::File::create(hosts_entry.path())
            .await
            .map_err(IoNodeError::create_fs_errmap(hosts_entry.path()))?;
