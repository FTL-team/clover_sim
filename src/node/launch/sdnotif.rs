...
use crate::node::{err::{NodeError, NodeResult}, workroot::WorkEntry};
...
impl SdNotifyListener {
    pub fn new(targ: WorkEntry) -> NodeResult<Self> {
        let sock =
            UnixDatagram::bind(targ.path()).map_err(NodeError::create_fs_errmap(targ.path()))?;
    ...
    pub async fn recv(&mut self) -> NodeResult<SdNotifyMessage> {
        loop {
            let recieved_n = self
                ...
                .await
                .map_err(NodeError::create_io_errmap("<sd_notify sock>"))?;
