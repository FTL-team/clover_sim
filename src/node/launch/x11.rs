use std::{env, os::unix::prelude::PermissionsExt, path::PathBuf, process::Stdio, time::Duration};
use tokio::{
...
impl X11 {
    ...
    pub async fn create_container_xauth(
        ...
        tokio::fs::File::create(xauthority_entry.path())
            .await
            .map_err(NodeError::create_fs_errmap(xauthority_entry.path()))?;

        tokio::fs::write(xauth_cmd_entry.path(), xauth_commands)
            .await
            .map_err(NodeError::create_fs_errmap(xauthority_entry.path()))?;
