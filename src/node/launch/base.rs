...
use crate::node::{cmd::simple_cmd, err::{NodeError, NodeResult}, workroot::WorkEntry};
...
fn match_oci_spec_error(e: oci_spec::OciSpecError, path: &PathBuf) -> NodeError {
    match e {
        ...
        oci_spec::OciSpecError::Io(err) => NodeError::create_fs_error(path, err),
...
impl BaseContainer {
    pub async fn start(options: BaseContainerOptions<'_>) -> NodeResult<BaseContainer> {
        ...
        let mount = Mount::builder()
            ...
            .mount_autodrop("overlay", fs_path, UnmountFlags::DETACH)
            .map_err(NodeError::create_io_errmap("mount<container overlay>"))?;
        ...
        let spec = Self::modify_spec(spec, &options, fs_path).map_err(&oci_err_map)?;
        ...
        let mut child = cmd
            .spawn()
            .map_err(NodeError::create_io_errmap("<container launch>"))?;
    ...
    pub async fn wait(&mut self) -> NodeResult<ExitStatus> {
        let res = self
            ...
            .wait()
            .await
            .map_err(NodeError::create_io_errmap("<container wait>"));
