    ...
    pub async fn run(rpc: Arc<dyn NodeRpc>, mut ctrl_rx: Receiver<bool>) -> Result<(), CliError> {
        ...
        loop {
            select! {
                        ...
                        Ok(line) => {
                            ...
                            match parts[0] {
                                ...
                                "tpath" => {
                                    ...
                                    match rpc.path_task_file(String::from(file)).await {
                                        Ok(file) => writeln!(stdout, "Path to task file: {}", file)?,
                                ...
                                "tread" => {
                                    let file = *parts.get(1).unwrap_or(&"README.md");
                                    match rpc.read_task_file(String::from(file)).await {
                                        Ok(contents) => {
                ...
                event = event_rx.recv() => {
                    let event = event.map_err(|x| CliError::InternalError(x.to_string()))?;
